...
protocol JSONableItem{
    static func parse(json: Any) -> Self?
    var json: Any { get}
    var id: String { get }
}

final class FileCacheSD {
    ...
    public init() {
        self.dataManager = try? SwiftDataManager()
    }
    ...
    func fetchAllItems() -> [TodoItem] {
        guard let items = dataManager?.fetch() else {return []}
        return items
    }
    ...
    private func checkFileExisting(filename: String) -> Bool {
